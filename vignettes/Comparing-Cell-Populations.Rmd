...
getCellTypeMarkers2 <- function(STObj, outputGRanges = TRUE, threshold = 12, numCores = 25) {
  ...
  allDifs <- lapply(names(assayList), function(x) {

    colData(newObj)$Comparison <- ifelse(colData(newObj)$CellType == x, 'Foreground', 'Background')

    getDifferentialAccessibleTiles(newObj,
      cellPopulation = 'allCells',
      groupColumn = "Comparison",
      foreground = "Foreground",
      background = "Background",
