...
getAltTSS <- function(completeDAPs,
  ...
  if(!all(colnames(GenomicRanges::mcols(DAP_GRanges)) %in% c('FDR','Log2FC_C'))){
  
    stop('GRanges input does not include columns for FDR and Log2FC_C, and so alternatively regulated TSSs cannot be identified. Please provide an object with those columns for filtering.')
  }
